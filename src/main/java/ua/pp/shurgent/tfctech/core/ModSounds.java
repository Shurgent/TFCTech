package ua.pp.shurgent.tfctech.core;

public class ModSounds {
	private static final String PATH = ModDetails.ModID + ":";

	public static final String GROOVEFIT = PATH + "rubbertapping.groove.fit";
	public static final String MOUNTFIT = PATH + "rubbertapping.mount.fit";
	public static final String BOWLFIT = PATH + "rubbertapping.bowl.fit";
	public static final String BOWLGRAB = PATH + "rubbertapping.bowl.grab";
	public static final String SCRATCH = PATH + "rubbertapping.trunk.scratch";
}
